...
private:
  ...
  void update( Time const&, const nest_long_t, const nest_long_t ) override;
...
template < class TGainfunction >
void
binary_neuron< TGainfunction >::update( Time const& origin, const nest_long_t from, const nest_long_t to )
{
  for ( nest_long_t lag = from; lag < to; ++lag )
  {
...
binary_neuron< TGainfunction >::handle( SpikeEvent& e )
{
  ...
  const nest_long_t m = e.get_multiplicity();
  const nest_long_t node_id = e.retrieve_sender_node_id_from_source_table();
