...
void
nest::aeif_cond_alpha_astro::update( Time const& origin, const nest_long_t from, const nest_long_t to )
{
  assert( State_::V_M == 0 );
...
nest::aeif_cond_alpha_astro::handle( SICEvent& e )
{
  ...
  const nest_long_t delay = e.get_delay_steps() - kernel().connection_manager.get_min_delay();
