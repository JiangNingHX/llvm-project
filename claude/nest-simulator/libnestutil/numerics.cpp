...
ld_round( double x )
{
  return static_cast< nest_long_t >( std::floor( x + 0.5 ) );
}
...
nest_long_t
mod_inverse( nest_long_t a, nest_long_t m )
{
  /*
  ...
  const nest_long_t a_orig = a;
  const nest_long_t m_orig = m;
  ...
  while ( a > 0 )
  {
    ...
    const nest_long_t s_0_new = -res.quot * s_0 + s_1;
    s_1 = s_0;
    s_0 = s_0_new;
  }
...
size_t
first_index( nest_long_t period, nest_long_t phase0, nest_long_t step, nest_long_t phase )
{
  assert( period > 0 );
  assert( step > 0 );
  ...
  /*
   ...
   We do all calculations in signed nest_long_t since we may encounter negative
  ...
  const nest_long_t d_phase = ( phase - phase0 + period ) % period;
  const nest_long_t d = std::gcd( step, period );
  ...
  const nest_long_t period_d = period / d;
  const nest_long_t step_d = step / d;
  const nest_long_t d_phase_d = d_phase / d;

  // Compute k_0 and multiply by step, see explanation in introductory comment
  const nest_long_t idx = ( d_phase_d * mod_inverse( step_d, period_d ) * step ) % std::lcm( period, step );
