...
constexpr uint8_t NUM_BITS_RANK = 17U;
...
#endif
constexpr uint8_t NUM_BITS_LCID = 29U;
...
/**
 *  Unsigned nest_long_t type for enumerations.
 */
[[maybe_unused]] constexpr size_t invalid_index = std::numeric_limits< size_t >::max();
...
[[maybe_unused]] const size_t max_targetindex = invalid_targetindex - 1;
 ...
 */
constexpr nest_long_t delay_min = std::numeric_limits< nest_long_t >::min();
constexpr nest_long_t delay_max = std::numeric_limits< nest_long_t >::max();
