...
class ScopedDWPAccumulatedTimer {
public:
  explicit ScopedDWPAccumulatedTimer(double *TotalSeconds)
      : TotalSeconds(TotalSeconds),
        Enabled(TotalSeconds != nullptr && isDWPStageTimingEnabled()),
        Start(std::chrono::steady_clock::now()) {}

  ~ScopedDWPAccumulatedTimer() {
    if (!Enabled)
      return;
...
struct SkeletonUnitTimingStats {
  double CreateInputContext = 0.0;
  double CollectExecutableRanges = 0.0;
  double ResolveSplitUnitDIE = 0.0;
  double CollectUnitMetadata = 0.0;
  double CollectUnitRanges = 0.0;
  double CollectSubprograms = 0.0;
  double ExpandRetainedReferenceClosure = 0.0;
  double FinalizeLiveUnitInfo = 0.0;
  double SubprogramAddressRanges = 0.0;
  double SubprogramNameLookup = 0.0;
  double SubprogramRangeCopies = 0.0;
  double SubprogramRetention = 0.0;
  double SubprogramRetainedChain = 0.0;
  double SubprogramRetainedSubtree = 0.0;
  double SubprogramReferenceClosure = 0.0;
  double SubprogramAppendLiveRanges = 0.0;
  double SubprogramStorage = 0.0;
  uint64_t NumUnits = 0;
  uint64_t NumVisitedDIEs = 0;
  uint64_t NumAddressableSubprograms = 0;
  uint64_t NumLiveSubprograms = 0;

  void log() const {
    if (!isDWPStageTimingEnabled())
      return;

    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.detail: "
        "units={0} visited-dies={1} addressable-subprograms={2} "
        "live-subprograms={3}\n",
        NumUnits, NumVisitedDIEs, NumAddressableSubprograms, NumLiveSubprograms);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.create-context: "
        "{0:F3}s\n",
        CreateInputContext);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.executable-ranges: "
        "{0:F3}s\n",
        CollectExecutableRanges);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.resolve-split-die: "
        "{0:F3}s\n",
        ResolveSplitUnitDIE);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.unit-metadata: "
        "{0:F3}s\n",
        CollectUnitMetadata);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.unit-ranges: "
        "{0:F3}s\n",
        CollectUnitRanges);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.collect-subprograms: "
        "{0:F3}s\n",
        CollectSubprograms);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.expand-ref-closure: "
        "{0:F3}s\n",
        ExpandRetainedReferenceClosure);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.finalize: "
        "{0:F3}s\n",
        FinalizeLiveUnitInfo);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.subprogram-address-ranges: "
        "{0:F3}s\n",
        SubprogramAddressRanges);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.subprogram-name-lookup: "
        "{0:F3}s\n",
        SubprogramNameLookup);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.subprogram-range-copies: "
        "{0:F3}s\n",
        SubprogramRangeCopies);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.subprogram-retention: "
        "{0:F3}s\n",
        SubprogramRetention);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.subprogram-retained-chain: "
        "{0:F3}s\n",
        SubprogramRetainedChain);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.subprogram-retained-subtree: "
        "{0:F3}s\n",
        SubprogramRetainedSubtree);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.subprogram-reference-closure: "
        "{0:F3}s\n",
        SubprogramReferenceClosure);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.subprogram-append-live-ranges: "
        "{0:F3}s\n",
        SubprogramAppendLiveRanges);
    errs() << formatv(
        "[llvm-dwarfutil][dwp-timing] collect-skeleton-units.subprogram-storage: "
        "{0:F3}s\n",
        SubprogramStorage);
  }
};
...
struct LoadedRetainedDWPFile {
  explicit LoadedRetainedDWPFile(OwningBinary<Binary> DWPBinary)
  ...
  ObjectFile &getObjectFile() {
    return *cast<ObjectFile>(DWPBinary.getBinary());
  }
...
static Expected<LoadedRetainedDWPFile>
loadRetainedDWPFile(StringRef DWPFileName, bool ThreadSafe,
                    DenseMap<DWARFSectionKind, uint64_t> *SectionSizes = nullptr) {
  Expected<OwningBinary<Binary>> DWPBinOrErr = openBinary(DWPFileName);
  if (!DWPBinOrErr)
    return DWPBinOrErr.takeError();

  LoadedRetainedDWPFile Loaded(std::move(*DWPBinOrErr));
  auto RecoverableHandler = [](Error Err) { consumeError(std::move(Err)); };
  Loaded.DWPContext = DWARFContext::create(
      Loaded.getObjectFile(), DWARFContext::ProcessDebugRelocations::Ignore,
      nullptr, "", RecoverableHandler, RecoverableHandler, ThreadSafe);
  ...
  if (Error Err = collectDWPSectionContents(Loaded.getObjectFile(),
...
static auto withThreadLocalRetainedDWPFile(StringRef DWPFileName, FnT &&Fn)
    -> decltype(Fn(std::declval<LoadedRetainedDWPFile &>())) {
  using ReturnT = decltype(Fn(std::declval<LoadedRetainedDWPFile &>()));
  thread_local std::string LoadedDWPFileName;
  thread_local std::unique_ptr<LoadedRetainedDWPFile> LoadedDWP;

  if (!LoadedDWP || LoadedDWPFileName != DWPFileName) {
    Expected<LoadedRetainedDWPFile> LoadedDWPOrErr =
        loadRetainedDWPFile(DWPFileName, /*ThreadSafe=*/false);
    if (!LoadedDWPOrErr)
      return ReturnT(LoadedDWPOrErr.takeError());

    LoadedDWP = std::make_unique<LoadedRetainedDWPFile>(
        std::move(*LoadedDWPOrErr));
    LoadedDWPFileName = DWPFileName.str();
  }
...
static bool isRetainedReferenceAttribute(dwarf::Attribute Attr) {
  switch (Attr) {
  case dwarf::DW_AT_type:
  case dwarf::DW_AT_specification:
  case dwarf::DW_AT_abstract_origin:
...
static void forEachRetainedReferenceDie(const DWARFDie &Die,
                                        CallbackT &&Callback) {
  for (const DWARFAttribute &AttrValue : Die.attributes()) {
    if (!isRetainedReferenceAttribute(AttrValue.Attr))
      continue;

    DWARFDie ReferencedDie = Die.getAttributeValueAsReferencedDie(AttrValue.Value);
    if (ReferencedDie)
      Callback(ReferencedDie);
  }
}

static void addRetainedDIESubtreeAndReferenceClosure(
    const DWARFDie &RootDie, SkeletonUnitInfo &Info,
    DenseSet<uint64_t> &SeenOffsets, DenseSet<uint64_t> &ExpandedSubtreeOffsets,
    DenseSet<uint64_t> &ExpandedReferenceOffsets,
    SkeletonUnitTimingStats *TimingStats = nullptr,
    DenseSet<uint64_t> *ScratchSeenDIEOffsets = nullptr,
    SmallVectorImpl<std::pair<DWARFDie, bool>> *ScratchWorklist = nullptr) {
  DenseSet<uint64_t> LocalSeenDIEOffsets;
  SmallVector<std::pair<DWARFDie, bool>, 8> LocalWorklist;
  DenseSet<uint64_t> &SeenDIEOffsets =
      ScratchSeenDIEOffsets ? *ScratchSeenDIEOffsets : LocalSeenDIEOffsets;
  SmallVectorImpl<std::pair<DWARFDie, bool>> &Worklist =
      ScratchWorklist ? *ScratchWorklist : LocalWorklist;
  SeenDIEOffsets.clear();
  Worklist.clear();
  Worklist.emplace_back(RootDie, false);
  ...
  while (!Worklist.empty()) {
    ...
    const bool NeedSubtree = !ExpandedSubtreeOffsets.contains(Offset);
    const bool NeedReferences = !ExpandedReferenceOffsets.contains(Offset);
    if (!NeedSubtree && !NeedReferences)
      continue;

    if (NeedSubtree) {
      addRetainedDIEOffset(Offset, Info, SeenOffsets);
      Worklist.emplace_back(Die, true);
    }

    if (NeedReferences) {
      forEachRetainedReferenceDie(Die, [&](const DWARFDie &ReferencedDie) {
        addRetainedDIEChain(ReferencedDie, Info, SeenOffsets);
        uint64_t ReferencedOffset = getUnitRelativeDIEOffset(ReferencedDie);
        if (!ExpandedSubtreeOffsets.contains(ReferencedOffset) ||
            !ExpandedReferenceOffsets.contains(ReferencedOffset))
          Worklist.emplace_back(ReferencedDie, false);
      });
...
collectRetainedDWPUnitTask(const DWPLinkMap &LinkMap, DWARFContext &DWPContext,
                           const RetainedDWPCollectUnitDescriptor &Descriptor) {
  Expected<DWARFUnit *> CUOrErr = getIndexedDWPCompileUnit(
      DWPContext, Descriptor.DWOId, Descriptor.UnitOffset,
      Descriptor.PackageUnitIndex);
  if (!CUOrErr)
    return CUOrErr.takeError();

  DWARFDie UnitDIE = (*CUOrErr)->getUnitDIE(/*ExtractUnitDIEOnly=*/false);
  if (!UnitDIE)
    return createStringError(
        std::errc::invalid_argument,
        formatv("unable to load retained package unit DIE for DWO_id {0:x16}",
  ...
  return collectRetainedDWPUnitInfo(LinkMap, UnitDIE);
}
...
collectRetainedDWPPackageUnitsInParallel(const DWPLinkMap &LinkMap,
  ...
  ScopedDWPStageTimer Timer("collect-retained-dwp-package-units-parallel");
  ...
  if (Error Err = executeRetainedDWPUnitTasks(
          ...
          [&](size_t, const RetainedDWPCollectUnitDescriptor &Descriptor)
              -> Expected<RetainedDWPUnitInfo> {
            return withThreadLocalRetainedDWPFile(
                DWPFileName,
                [&](LoadedRetainedDWPFile &LoadedDWP)
                    -> Expected<RetainedDWPUnitInfo> {
                  return collectRetainedDWPUnitTask(
                      LinkMap, *LoadedDWP.DWPContext, Descriptor);
                });
          }))
    return std::move(Err);
...
analyzeRetainedDWPRewriteUnitTask(
    const DWPLinkMap &LinkMap, DWARFContext &DWPContext,
    const DenseMap<DWARFSectionKind, StringRef> &SectionContents,
    const RetainedDWPPackageUnitInfo &RetainedUnit,
    const RetainedDWPRewriteUnitDescriptor &Descriptor) {
  Expected<DWARFUnit *> CUOrErr = getRetainedDWPCompileUnit(DWPContext, Descriptor);
  if (!CUOrErr)
    return CUOrErr.takeError();
  ...
  return analyzeRetainedDWPRewriteUnit(
      LinkMap, RetainedUnit.Unit, UnitDIE,
...
rewriteRetainedDWPUnitTask(
  ...
  return rewriteRetainedDWPUnit(LinkMap, UnitDIE, SectionContents, Analysis,
                                StringOffsetRemap);
}

Expected<RetainedDWPRewritePlan>
collectRetainedDWPRewritePlan(const DWPLinkMap &LinkMap, const Options &Options) {
  ScopedDWPStageTimer Timer("collect-retained-dwp-rewrite-plan");
  ...
  Expected<LoadedRetainedDWPFile> LoadedDWPOrErr = [&]() {
    ScopedDWPStageTimer LoadTimer("collect-retained-dwp-rewrite-plan.load-dwp");
    return loadRetainedDWPFile(Options.DWPFileName, /*ThreadSafe=*/false,
                               &SectionSizes);
  }();
  if (!LoadedDWPOrErr)
    return LoadedDWPOrErr.takeError();
  DWARFContext &DWPContext = *LoadedDWPOrErr->DWPContext;
  const DenseMap<DWARFSectionKind, StringRef> &SectionContents =
      LoadedDWPOrErr->SectionContents;
  StringRef DebugStrContents = LoadedDWPOrErr->DebugStrContents;
  ...
  if (UseParallel) {
    ScopedDWPStageTimer AnalyzeTimer(
        "collect-retained-dwp-rewrite-plan.analyze-units");
              ...
              return withThreadLocalRetainedDWPFile(
                  ...
                  [&](LoadedRetainedDWPFile &LoadedDWP)
                    ...
                    return analyzeRetainedDWPRewriteUnitTask(
                        LinkMap, *LoadedDWP.DWPContext,
                        LoadedDWP.SectionContents,
  ...
  Expected<RetainedDWPStringSectionInfo> RewrittenStringSectionOrErr = [&]() {
    ScopedDWPStageTimer StringTimer(
        "collect-retained-dwp-rewrite-plan.build-string-section");
    return buildRetainedDWPStringSection(Descriptors, DebugStrContents,
                                         UnitAnalyses);
  }();
  ...
  if (UseParallel) {
    ScopedDWPStageTimer RewriteTimer(
        "collect-retained-dwp-rewrite-plan.rewrite-units");
              ...
              return withThreadLocalRetainedDWPFile(
                  ...
                  [&](LoadedRetainedDWPFile &LoadedDWP)
                    ...
                    return rewriteRetainedDWPUnitTask(
                        LinkMap, *LoadedDWP.DWPContext,
                        LoadedDWP.SectionContents, Descriptor, Analysis,
  ...
  {
    ScopedDWPStageTimer MaterializeTimer(
        "collect-retained-dwp-rewrite-plan.materialize-output");
    ...
    for (size_t I = 0; I != Plan.Units.size(); ++I) {
      ...
      for (const PackageUnitInfo::SectionContributionInfo &Contribution :
        ...
        if (std::optional<StringRef> SectionName =
                getDWPSectionName(Contribution.Kind))
        ...
        if (std::optional<StringRef> RewrittenContents =
                getRewrittenUnitContents(Contribution.Kind,
                                         RewrittenUnitContents))
    ...
    for (RetainedDWPSectionContributionInfo &Contribution :
         Plan.Contributions) {
    ...
    for (DWARFSectionKind Kind : {DW_SECT_INFO, DW_SECT_ABBREV,
      ...
      RetainedDWPOutputRecord &Record =
          Plan.OutputManifest.Records.emplace_back();
...
static Error collectSubprograms(DWARFDie RootDie,
                                ...
                                DenseSet<uint64_t> &ExpandedSubtreeOffsets,
                                DenseSet<uint64_t> &ExpandedReferenceOffsets,
                                bool CollectVerboseDetails,
                                SkeletonUnitTimingStats *TimingStats = nullptr) {
  SmallVector<DWARFDie, 8> Worklist;
  DenseSet<uint64_t> RetainedOffsets;
  DenseSet<uint64_t> ReferenceSeenDIEOffsets;
  SmallVector<std::pair<DWARFDie, bool>, 8> ReferenceWorklist;
  Worklist.push_back(RootDie);

  while (!Worklist.empty()) {
    DWARFDie Die = Worklist.pop_back_val();
    if (TimingStats)
      ++TimingStats->NumVisitedDIEs;
    if (Die.isSubprogramDIE()) {
      Expected<DWARFAddressRangesVector> RangesOrErr = [&]() {
        ScopedDWPAccumulatedTimer Timer(
            TimingStats ? &TimingStats->SubprogramAddressRanges : nullptr);
        return Die.getAddressRanges();
      }();
      if (!RangesOrErr)
        return RangesOrErr.takeError();
      if (!RangesOrErr->empty()) {
        if (TimingStats)
          ++TimingStats->NumAddressableSubprograms;
        SmallVector<SkeletonUnitInfo::AddressRangeInfo, 2> LiveRanges;
        SkeletonUnitInfo::SubprogramInfo Subprogram;
        if (CollectVerboseDetails) {
          Subprogram.Offset = getUnitRelativeDIEOffset(Die);
          {
            ScopedDWPAccumulatedTimer Timer(
                TimingStats ? &TimingStats->SubprogramNameLookup : nullptr);
            Subprogram.Name =
          ...
          {
            ScopedDWPAccumulatedTimer Timer(
                TimingStats ? &TimingStats->SubprogramRangeCopies : nullptr);
            copyAddressRanges(Subprogram.ResolvedRanges, *RangesOrErr);
            keepLiveRanges(ExecutableRanges, Subprogram.LiveRanges, *RangesOrErr);
          }
          LiveRanges.append(Subprogram.LiveRanges.begin(), Subprogram.LiveRanges.end());
        } else {
          ScopedDWPAccumulatedTimer Timer(
              TimingStats ? &TimingStats->SubprogramRangeCopies : nullptr);
          keepLiveRanges(ExecutableRanges, LiveRanges, *RangesOrErr);
        }

        if (!LiveRanges.empty()) {
          if (TimingStats)
            ++TimingStats->NumLiveSubprograms;
          Info.LiveRootOffsets.push_back(getUnitRelativeDIEOffset(Die));
          {
            ScopedDWPAccumulatedTimer Timer(
                TimingStats ? &TimingStats->SubprogramRetention : nullptr);
            {
              ScopedDWPAccumulatedTimer Timer(
                  TimingStats ? &TimingStats->SubprogramRetainedChain
                              : nullptr);
              addRetainedDIEChain(Die, Info, RetainedOffsets);
            }
            {
              ScopedDWPAccumulatedTimer Timer(
                  TimingStats ? &TimingStats->SubprogramReferenceClosure
                              : nullptr);
              const uint64_t Offset = getUnitRelativeDIEOffset(Die);
              if (!ExpandedSubtreeOffsets.contains(Offset) ||
                  !ExpandedReferenceOffsets.contains(Offset)) {
                addRetainedDIESubtreeAndReferenceClosure(
                    Die, Info, RetainedOffsets, ExpandedSubtreeOffsets,
                    ExpandedReferenceOffsets, TimingStats,
                    &ReferenceSeenDIEOffsets, &ReferenceWorklist);
              }
            }
            {
              ScopedDWPAccumulatedTimer Timer(
                  TimingStats ? &TimingStats->SubprogramAppendLiveRanges
                              : nullptr);
        ...
        if (CollectVerboseDetails) {
          ScopedDWPAccumulatedTimer Timer(
              TimingStats ? &TimingStats->SubprogramStorage : nullptr);
...
struct RetainedDWPOutputTreeNodeInfo {
  dwarf::Tag Tag = dwarf::DW_TAG_null;
  uint32_t Depth = 0;
};

static SmallVector<RetainedDWPOutputTreeNodeInfo, 16>
collectRetainedDWPOutputTreeNodeInfos(const DWARFDie &RootDie) {
  SmallVector<RetainedDWPOutputTreeNodeInfo, 16> Infos;
  SmallVector<std::pair<DWARFDie, uint32_t>, 8> Worklist;
  Worklist.emplace_back(RootDie, 0);

  while (!Worklist.empty()) {
    auto [Die, Depth] = Worklist.pop_back_val();
    Infos.push_back({Die.getTag(), Depth});

    for (DWARFDie Child : reverse(Die.children()))
      Worklist.emplace_back(Child, Depth + 1);
  }

  return Infos;
}

static Expected<SmallVector<RetainedDWPOutputTreeNodeInfo, 16>>
collectExpectedRetainedDWPOutputTreeNodeInfos(
    const RetainedDWPUnitInfo &RetainedUnit) {
  SmallVector<RetainedDWPOutputTreeNodeInfo, 16> Infos;
  DenseMap<uint64_t, uint32_t> DepthByOffset;
  Infos.reserve(RetainedUnit.RetainedDIEs.size());

  for (const RetainedDWPDieInfo &RetainedDie : RetainedUnit.RetainedDIEs) {
    uint32_t Depth = 0;
    if (RetainedDie.ParentOffset) {
      auto ParentIt = DepthByOffset.find(*RetainedDie.ParentOffset);
      if (ParentIt == DepthByOffset.end())
        return createStringError(
            std::errc::invalid_argument,
            formatv("missing retained parent depth for DIE 0x{0:x} in DWO_id "
                    "{1:x16}",
                    RetainedDie.Offset, RetainedUnit.DWOId)
                .str()
                .c_str());
      Depth = ParentIt->second + 1;
    }

    DepthByOffset.try_emplace(RetainedDie.Offset, Depth);
    Infos.push_back({RetainedDie.Tag, Depth});
  }

  return Infos;
}

static Error validateRetainedDWPOutputUnit(
    const RetainedDWPPackageUnitInfo &RetainedPackageUnit,
    const DWARFDie &RootDie) {
  Expected<SmallVector<RetainedDWPOutputTreeNodeInfo, 16>> ExpectedNodesOrErr =
      collectExpectedRetainedDWPOutputTreeNodeInfos(RetainedPackageUnit.Unit);
  if (!ExpectedNodesOrErr)
    return ExpectedNodesOrErr.takeError();

  SmallVector<RetainedDWPOutputTreeNodeInfo, 16> ActualNodes =
      collectRetainedDWPOutputTreeNodeInfos(RootDie);
  if (ActualNodes.size() != ExpectedNodesOrErr->size())
    return createStringError(
        std::errc::invalid_argument,
        formatv("retained DIE count mismatch for DWO_id {0:x16}: {1} DIEs vs "
                "{2} expected nodes",
                RetainedPackageUnit.Unit.DWOId, ActualNodes.size(),
                ExpectedNodesOrErr->size())
            .str()
            .c_str());

  for (size_t I = 0; I != ActualNodes.size(); ++I) {
    const RetainedDWPOutputTreeNodeInfo &ExpectedNode =
        (*ExpectedNodesOrErr)[I];
    if (ActualNodes[I].Tag == ExpectedNode.Tag &&
        ActualNodes[I].Depth == ExpectedNode.Depth)
      continue;
    return createStringError(
        std::errc::invalid_argument,
        formatv("retained DIE structure mismatch for DWO_id {0:x16} at index "
                "{1}: expected tag={2} depth={3}, got tag={4} depth={5}",
                RetainedPackageUnit.Unit.DWOId, I,
                dwarf::TagString(ExpectedNode.Tag), ExpectedNode.Depth,
                dwarf::TagString(ActualNodes[I].Tag), ActualNodes[I].Depth)
            .str()
            .c_str());
  }
...
static Error resolveSplitUnitInfo(DWARFUnit &SkeletonUnit,
  ...
  DWARFDie SplitUnitDie = [&]() {
    ScopedDWPAccumulatedTimer Timer(
        TimingStats ? &TimingStats->ResolveSplitUnitDIE : nullptr);
    return SkeletonUnit.getNonSkeletonUnitDIE(
  ...
  if (CollectVerboseDetails) {
    ScopedDWPAccumulatedTimer Timer(
        TimingStats ? &TimingStats->CollectUnitMetadata : nullptr);
    Info.SplitUnitName =
  ...
  if (CollectVerboseDetails) {
    Expected<DWARFAddressRangesVector> RangesOrErr = [&]() {
      ScopedDWPAccumulatedTimer Timer(
          TimingStats ? &TimingStats->CollectUnitRanges : nullptr);
      return SplitUnitDie.getAddressRanges();
    }();
    if (!RangesOrErr)
      return createStringError(
          std::errc::invalid_argument,
  ...
  {
    ScopedDWPAccumulatedTimer Timer(
        TimingStats ? &TimingStats->ExpandRetainedReferenceClosure : nullptr);
    expandRetainedReferenceClosure(SplitUnitDie, Info, ExpandedSubtreeOffsets,
                                   ExpandedReferenceOffsets);
  }
  {
    ScopedDWPAccumulatedTimer Timer(
        TimingStats ? &TimingStats->FinalizeLiveUnitInfo : nullptr);
...
Expected<DWPLinkMap> loadDWPLinkMap(const object::ObjectFile &InputFile,
  ...
  ScopedDWPStageTimer Timer("load-dwp-link-map");
  DWPLinkMap LinkMap;
  std::unique_ptr<DWARFContext> InputContext;
  SkeletonUnitTimingStats SkeletonTimingStats;
  {
    ScopedDWPStageTimer StageTimer("load-dwp-link-map.collect-skeleton-units");
    {
      ScopedDWPAccumulatedTimer Timer(&SkeletonTimingStats.CreateInputContext);
      InputContext = DWARFContext::create(InputFile);
    }
    AddressRanges ExecutableRanges = [&]() {
      ScopedDWPAccumulatedTimer Timer(
          &SkeletonTimingStats.CollectExecutableRanges);
      return collectExecutableAddressRanges(InputFile);
    }();
    if (Error Err = collectSkeletonUnits(*InputContext, LinkMap,
                                         ExecutableRanges, Options.DWPFileName,
                                         Options.Verbose,
                                         &SkeletonTimingStats))
      return std::move(Err);
  }
  SkeletonTimingStats.log();
  ...
  Expected<LoadedRetainedDWPFile> LoadedDWPOrErr = [&]() {
    ScopedDWPStageTimer StageTimer("load-dwp-link-map.open-dwp");
    return loadRetainedDWPFile(Options.DWPFileName, /*ThreadSafe=*/false);
  }();
  if (!LoadedDWPOrErr)
    return LoadedDWPOrErr.takeError();
  ObjectFile &DWPObject = LoadedDWPOrErr->getObjectFile();
  DWARFContext &DWPContext = *LoadedDWPOrErr->DWPContext;
  ...
  {
    ScopedDWPStageTimer StageTimer("load-dwp-link-map.collect-package-units");
  ...
  Expected<SmallVector<RetainedDWPPackageUnitInfo, 4>> RetainedUnitsOrErr =
      [&]() -> Expected<SmallVector<RetainedDWPPackageUnitInfo, 4>> {
    ScopedDWPStageTimer StageTimer("load-dwp-link-map.collect-retained-units");
    return shouldUseRetainedDWPRewriteParallelism(Options,
                                                  LinkMap.LinkedUnits.size())
               ? collectRetainedDWPPackageUnitsInParallel(
                     LinkMap, DWPContext, Options, Options.DWPFileName)
  ...
  Expected<RetainedDWPRewritePlan> RewritePlanOrErr = [&]() {
    ScopedDWPStageTimer StageTimer("load-dwp-link-map.collect-rewrite-plan");
    return collectRetainedDWPRewritePlan(LinkMap, Options);
  }();
  if (!RewritePlanOrErr)
  ...
  {
    ScopedDWPStageTimer StageTimer("load-dwp-link-map.validate-rewrite-plan");
...
Error validateRetainedDWPOutputContainerLinkage(const Options &Options,
                                                const DWPLinkMap &LinkMap) {
  ScopedDWPStageTimer Timer("validate-retained-dwp-output-container-linkage");
  Expected<OwningBinary<Binary>> OutputBinOrErr = openBinary(Options.OutputFileName);
  if (!OutputBinOrErr)
    return OutputBinOrErr.takeError();

  auto *OutputObject = cast<ObjectFile>(OutputBinOrErr->getBinary());
  std::unique_ptr<DWARFContext> OutputContext = DWARFContext::create(*OutputObject);
  if (!OutputContext->isDWP())
    return createStringError(
        std::errc::invalid_argument,
        formatv("file '{0}' is not a DWARF package file", Options.OutputFileName)
            .str()
            .c_str());

  DenseSet<uint64_t> SeenDWOIds;
  size_t RevalidatedUnits = 0;

  for (const std::unique_ptr<DWARFUnit> &CU : OutputContext->dwo_compile_units()) {
    std::optional<uint64_t> DWOId = CU->getDWOId();
    if (!DWOId)
      return createStringError(
          std::errc::invalid_argument,
          formatv("output DWP compile unit '{0}' is missing DWO_id",
                  getUnitName(*CU))
              .str()
              .c_str());

    if (!SeenDWOIds.insert(*DWOId).second)
      return createStringError(
          std::errc::invalid_argument,
          formatv("duplicate output DWP compile unit for DWO_id {0:x16}", *DWOId)
              .str()
              .c_str());

    if (!LinkMap.findLinkedUnit(*DWOId))
      return createStringError(
          std::errc::invalid_argument,
          formatv("unexpected output DWP compile unit for DWO_id {0:x16}", *DWOId)
              .str()
              .c_str());
    const RetainedDWPPackageUnitInfo *RetainedPackageUnit =
        LinkMap.findRetainedPackageUnit(*DWOId);
    if (!RetainedPackageUnit)
      return createStringError(
          std::errc::invalid_argument,
          formatv("missing retained output package unit for DWO_id {0:x16}",
                  *DWOId)
              .str()
              .c_str());
    ...
    if (!UnitDIE)
      return createStringError(
          std::errc::invalid_argument,
          formatv("unable to load emitted DWP compile unit DIE for DWO_id {0:x16}",
                  *DWOId)
              .str()
              .c_str());

    if (Error Err = validateLiveSubprograms(LinkMap, UnitDIE))
      return Err;
    if (Error Err = validateRetainedDWPOutputUnit(*RetainedPackageUnit, UnitDIE))
      return Err;
    ++RevalidatedUnits;
  }
