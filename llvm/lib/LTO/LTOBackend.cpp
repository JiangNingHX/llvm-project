...
static bool splitOptAndCodeGenThin(unsigned task, const Config &C,
  ...
  if (!C.AcceptsMultipleOutputsPerTask && SplitModuleCG.getPartitionNum() > 1) {
    report_fatal_error("The current LTO client does not support ThinLTO split codegen.");
  }
  ...
  const auto HandleModulePartition = [&](std::unique_ptr<Module> MPart,
    ...
    if (DoOpt) {
      if (!opt(C, ThreadTM.get(), task, *MPart, /*IsThinLTO=*/true,
    ...
    codegen(C, ThreadTM.get(), AddStream, task, *MPart, CombinedIndex);
    if (NeedLocalProfiler) {
